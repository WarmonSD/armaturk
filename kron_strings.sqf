...
KRON_StrRight = {
    ...
    for "_i"
    from((count _arr) - _len) to((count _arr) - 1) do {
        _out = _out + (_arr select _i);
    };
    _out
};
...
KRON_StrIndex = {
    ...
    for "_i"
    from 0 to(_lh - _ln) do {
        _tmp = "";
        for "_j"
        from _i to(_i + _ln - 1) do {
                _tmp = _tmp + (_arr select _j);
            };
            if (_tmp == _ndl) exitWith {
                _out = _i
            };
    };
    _out
...
KRON_StrInStr = {
    ...
    _in = _this select 0;
    _out =
    if (([_this select 0, _this select 1] call KRON_StrIndex) == -1) then {
        false
    } else {
        true
    };
...
KRON_Compare = {
    ...
    from 0 to 1 do {
        ...
        switch (typeName _t) do {
            ...
            case "STRING":{
                    if !(_k) then {
                        _this = [_this] call KRON_ArrayToUpper
                    }
                };
            default {
                _n = -1
...
KRON_ArraySort = {
    ...
    _a = +(_this select 0);
    _d =
    if ([_this, "DESC"] call KRON_findFlag) then {
        -1
    } else {
        1
    };
    _k =
    if ([_this, "CASE"] call KRON_findFlag) then {
        "CASE"
    } else {
        "nocase"
    };
