...
waitUntil {
    !isNil "life_HC_isActive" && {!isNil "life_server_extDB_notLoaded"
    }
};
...
[] call life_fnc_guvenli;
switch (playerSide) do {
    case west:{
            life_paycheck = getNumber(missionConfigFile >> "Life_Settings" >> "paycheck_cop");
        };
    case civilian:{
            life_paycheck = getNumber(missionConfigFile >> "Life_Settings" >> "paycheck_civ");
        };
    case independent:{
            life_paycheck = getNumber(missionConfigFile >> "Life_Settings" >> "paycheck_med");
        };
};
switch (playerSide) do {
    case west:{
...
life_paycheck = compileFinal(
    if (life_paycheck isEqualType "") then {
        life_paycheck
    } else {
        str(life_paycheck)
    });
if (getNumber(missionConfigFile >> "Life_Settings" >> "enable_fatigue") isEqualTo 0) then {
    player enableFatigue false;
};
if (getNumber(missionConfigFile >> "Life_Settings" >> "pump_service") isEqualTo 1) then {
...
[getPlayerUID player, player getVariable["realname", name player]] remoteExec["life_fnc_bountyProfUpdate", 2];
life_hideoutBuildings = []; {
    private _building = nearestBuilding getMarkerPos _x;
    life_hideoutBuildings pushBack _building;
    false
}
count["gang_area_1", "gang_area_3", "gang_area_4", "gang_area_5", "alikampos_1", "alikampos_2", "alikampos_3", "alikampos_4", "alikampos_5", "alikampos_6", "alikampos_7", "alikampos_8", "alikampos_9", "alikampos_10", "alikampos_11", "alikampos_12"];
DYNAMICMARKET_boughtItems = [];
[player] remoteExec["TON_fnc_playerLogged", 2];
onPlayerDisconnected {
    [_id, _name, _uid] call compile preProcessFileLineNumbers "core\functions\fn_onPlayerDisconnect.sqf"
};
diag_log "----------------------------------------------------------------------------------------------------";
